<template>
  <div class="navbar">
    ...
    <el-dialog title="修改头像" :visible.sync="dialogFormimg" width="400px">
      ...
      <div style="margin-top:20px">
        <el-button type="primary" @click="getupimg">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    upLoad(param) {
      upfile(param.file, "Upload/AddUpload?folder=ModifyImg&type=1", data => {
        if (data.Status) {
    ...
    getupimg: function() {
      ...
      request({
        ...
        if (response.Status == 1) {
          this.face = this.filepath;
          this.dialogFormimg = false;
        }
      });
    },
    getpassword() {
