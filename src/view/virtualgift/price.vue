  ...
  <div class="product boxright"> 
    ...
    <el-dialog title="设置供货价" :visible.sync="dialogAddVisible" :close-on-click-modal="false" width="850px">
       <el-card class="box-card" v-for="item in chiocelist" :key="item.Id">
        <div class="flex">
          <div class="left-one">名称：{{item.Name}}<br/>面值：{{item.Price}} - 进货价：{{item.PurchasePrice}}</div>
          <div>
  ...
  methods: {
    ...
    getList(){
      ...
      request({
        ...
        method: "get",
        params: {CId:this.$route.query.id,Type:0}
      }).then(response => {
        if (response.Status == 1) {
          ...
          cancelButtonText: '取消',
           ...
           request({
            ...
            if(response.Status==1){
              ...
              const index = biglist.indexOf(row)
    ...
    getchoicelist(){
      request({
        url: "Customer/GetProductCategoryList",
        method: "get",
        params: {CId:this.$route.query.id,Category:this.Category,}
      }).then(response => {
        if (response.Status == 1) {
          for(let i in response.List){
            response.List[i].setprice=response.List[i].Price;
          }
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {              
          ...
          request({
              ...
              if(response.Status==1){
                for(let i in biglist){
                    if (biglist[i].Id === this.temp.Id) {
                      biglist[i].XSPrice=this.temp.Price;
                      break
                    }  
                  }    
